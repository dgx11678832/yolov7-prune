...
cfg = '/home/deepblue/PycharmProjects/yolov7-prune/yolov7-main/cfg/training/yolov7.yaml'
device = torch.device('cpu')
path = 'yolov7.pt'
pruning_mthd = 'kernel_norm'
sparsity = [0.1 ,0.3,0.5]
all_scores = torch.zeros((6, len(sparsity)))
with torch.no_grad():
    model = attempt_load(path, map_location='cpu').to(device)
...
#
#
# model = torch.hub.load(GITHUB, 'yolov5s', pretrained=True, force_reload=True)
